  ...
  <el-dialog
    ...
    <div class="categoryPanel clearfix">
      <div class="categoryWrap">
        <div class="item" :class="{'active': activeLevl1.id === item.id}" v-for="(item, i) in lev1List" :key="i" @click="handleClick(item, 1)">
          {{item.name}}
        </div>
      </div>
      <div class="categoryWrap">
        <div class="item" :class="{'active': activeLevl2.id === item.id}" v-for="(item, i) in lev2List" :key="i"  @click="handleClick(item, 2)">
          <span>
      ...
      <div class="categoryWrap">
        <div class="item" :class="{'active': activeLevl3.id === item.id}" v-for="(item, i) in lev3List" :key="i" @click="handleClick(item, 3)">
          <el-checkbox v-model="item.checked" @change='handleChange(item, 3)' @click.native.stop></el-checkbox>
          <span class="name">{{item.name}}</span>
        </div>
      </div>
      <div class="categoryWrap">
        <div class="item" :class="{'active': activeLevl4.id === item.id}" v-for="(item, i) in lev4List" :key="i" @click="handleClick(item, 4)">
...
export default {
  ...
  // computed: {
  //   list () {
  //     return this.formatTree(this.categoryList)
  //   }
  // }
  watch: {
    categoryList: {
      handler (v) {
        this.lev1List = this.formatTree(v)
      },
      immediate: true
    }
  },
  ...
  methods: {
    /**
     * 点击当前行时触发，设置当前选中行,将子集的第一项设置为选中项，并设置子集及孙级展示内容，
     */
    handleClick (item, level, changeStatu) {
      ...
      // 设置子级及孙级类目框的当前显示
      this.setNextLevelList(item, level)
      // 如果有状态，将子级及孙级选中状态设置为该状态
      if (changeStatu) {
     ...
     */
    setNextLevelList (item, level) {
      if (level === 4) return
      level++
      this[`lev${level}List`] = item.children
      this[`activeLevl${level}`] = item.children.length && item.children[0]
      if (item.children.length) {
        this.setNextLevelList(item.children[0], level)
      }
    },
    /**
    ...
    handleChange (item, level) {
      this.handleClick(item, level, true)
    ...
    formatTree (list) {
      return list.map(v => {
        return {
          id: v.businessChargeId,
          name: v.businessCategoryName,
          children: this.formatTree(v.child),
          commissionPercentage: v.commissionPercentage,
          parentId: v.parentId,
          categoryMargin: v.categoryMargin,
          marginUnitId: v.marginUnitId
        }
      })
    }
  }
